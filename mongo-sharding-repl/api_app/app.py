...
async def root():
        ...
        try:
            ...
            if "shards" in coll_stats:
                for shard_name, shard_stats in coll_stats["shards"].items():
                    shards_count[shard_name] = shard_stats.get("count", 0)
        except Exception:
            pass
    ...
    if topology_type == "Sharded":
        ...
        for shard in shards_list.get("shards", {}):
            ...
            # Count replicas from host string (format: "replicaset/host1:port,host2:port,host3:port")
            if "/" in shard_host:
                hosts_part = shard_host.split("/")[1]
                replica_count = len(hosts_part.split(","))
                shards_replicas[shard_id] = replica_count
    ...
    return {
        ...
        "shards": shards,
        "shards_replicas": shards_replicas,
